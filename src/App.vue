...
/* ═══════════════════════════════════════════════════════════════
   MOBILE RESPONSIVE — make PC layout work on phones/tablets
   Target: phones in landscape (≤900px wide) and portrait (≤480px)
═══════════════════════════════════════════════════════════════ */

/* ── Landscape phones & small tablets (600–900px wide) ── */
@media (max-width: 900px) {
  /* Fix topbar — vw-based height is too large on tablets */
  .tetBar {
    height: clamp(50px, 6.5vh, 72px) !important;
    min-height: clamp(50px, 6.5vh, 72px) !important;
    max-height: clamp(50px, 6.5vh, 72px) !important;
  }

  /* Shrink left column so buttons get more right-side space */
  .mnLeft {
    flex: 0 0 clamp(140px, 30vw, 340px);
  }

  /* Make button stack wider to properly bleed off the right edge */
  .mnBtns {
    width: calc(70vw + 60px);
    margin-right: -60px;
  }

  /* Reduce left padding on menu container */
  .mnMenu {
    padding-left: clamp(16px, 3vw, 40px);
  }

  /* Auth screen: give right column more breathing room */
  .hpLeft {
...
/* ── Portrait phones (≤ 480px wide) ── */
@media (max-width: 480px) {
  /* Fix topbar to a proper fixed px height */
  .tetBar {
  ...
  /* Fix all vw-based top/bottom offsets to match fixed topbar */
  .mnMenu {
    top: 54px;
    bottom: 36px;
    padding-left: 12px;
  ...
  /* Bottom bars that use 5.208vw height */
  .hpBottomBar,
  .tetBottomBar {
    height: 36px;
  }

  /* Collapse left brand column — show tiny logo only */
  .mnLeft {
    flex: 0 0 72px;
  ...
  /* Buttons fill the rest of the screen, bleed off right edge */
  .mnBtns {
    width: calc(100vw - 84px + 60px);
    margin-right: -60px;
    gap: 10px;
  }

  /* Auth screen portrait: stack vertically */
  .hpAuth {
    flex-direction: column;
    align-items: stretch;
    justify-content: flex-start;
